...
import AnimationResult from '../animationResult/AnimationResult'
...
class TakePicture extends Component {
    ...
    saveImage = (e) => {
        ...
        prominent(imageToCheck, { amount: 1 }).then(color => {
             // [241, 221, 63]
            var hexColor = color.map(function (x) {
                x = parseInt(x).toString(16);
                return (x.length == 1) ? "0" + x : x;
            })
            hexColor = "#" + hexColor.join("");
            this.setState({ mainColor: hexColor })
            ntc.init();
            var n_match  = ntc.name(this.state.mainColor);
            let n_shade_name = n_match[3]; 
        ...
        if (this.state.save === true) {
            return (
                    <div>
                        ...
                        <Modal show={this.state.save} onHide={this.closeModal}>
                        <Modal.Header closeButton>
                        </Modal.Header>
                        <Modal.Body>
                            <AnimationResult res={this.state.right}/>
                        </Modal.Body>
                        <Modal.Footer>
